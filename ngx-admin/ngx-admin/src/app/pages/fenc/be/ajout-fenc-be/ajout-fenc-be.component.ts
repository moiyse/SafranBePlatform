...
@Component({
  selector: 'ngx-ajout-fenc-be',
  templateUrl: './ajout-fenc-be.component.html',
  styleUrls: ['./ajout-fenc-be.component.scss']
})
export class AjoutFencBeComponent implements OnInit {
  ...
  onSubmitJustification(){
    ...
    const {justification,limitation,nom,date,signature} = this.methodeFencForm.value;
    let anomalie = this.anomalie
    this.justificationToSend = {...this.justificationToSend,justification,limitation,nom,date,signature,anomalie}
    ...
    this.justificationService.addJustificationWithAnomalie(this.justificationToSend,this.justificationToSend.anomalie.idAnomalie).subscribe(data => {
      console.log(data)
      this.router.navigateByUrl("/pages/FENC/be/attenteFENC")
    })
  }
