...
@Component({
  selector: 'ngx-update-fenc-admin',
  templateUrl: './update-fenc-admin.component.html',
  styleUrls: ['./update-fenc-admin.component.scss']
})
export class UpdateFencAdminComponent implements OnInit {
    ...
    this.dialogService.open(DialogChoiceServiceComponent).onClose.subscribe((service:string) => {
      if(service){
        ...
        if(this.formFENC.valid){
          this.anomalieService.updateAnomalie(this.formFENC.value).subscribe(data => {
